...
// GET Purchase Order Details Report
router.get('/reports/purchase-order-details', async (req, res) => {
  try {
    ...
    if (from && to) {
      dateFilter = 'WHERE p.date >= $1 AND p.date <= $2';
      params = [from, to + 'T23:59:59.999'];
    }

    const result = await db.query(`
            SELECT p.id, p.status, p.date, p.expected_date, p.po_number,
    ...
    console.error('Error fetching purchase order details report:', err);
    res.status(500).json({ error: 'Failed to fetch purchase order details report' });
  }
});

// GET Purchase Orders by Vendor Report
router.get('/reports/purchase-orders-by-vendor', async (req, res) => {
  try {
    const db = database.getDb();
    const { from, to } = req.query;
    ...
    const result = await db.query(`
            SELECT COALESCE(s.name, p.supplier_name) AS vendor_name,
                   COUNT(p.id) AS purchase_order_count,
                   SUM(p.total_amount) AS amount
            FROM purchases p
            LEFT JOIN suppliers s ON p.supplier_id = s.id
            ${dateFilter}
            GROUP BY COALESCE(s.name, p.supplier_name)
            HAVING COALESCE(s.name, p.supplier_name) IS NOT NULL
            ORDER BY COALESCE(s.name, p.supplier_name) ASC
        `, params);

    res.json(result.rows);
  } catch (err) {
    console.error('Error fetching purchase orders by vendor report:', err);
    res.status(500).json({ error: 'Failed to fetch purchase orders by vendor report' });
  }
});

// GET Active Purchase Orders Report (Purchases by Item for active POs)
router.get('/reports/active-purchase-orders', async (req, res) => {
  try {
    const db = database.getDb();
    const { from, to } = req.query;
    ...
    if (from && to) {
      dateFilter = "AND p.date >= $1 AND p.date <= $2";
      params = [from, to + 'T23:59:59.999'];
    }

    const result = await db.query(`
            SELECT COALESCE(pi.item_name, i.name, 'Unknown Item') AS product_code,
                   SUM(pi.quantity) AS quantity_purchased,
                   ROUND(AVG(pi.unit_price)::numeric, 2) AS cost_price,
                   COALESCE(i.sku, '') AS sku
            FROM purchases p
            JOIN purchase_items pi ON pi.purchase_id = p.id
            LEFT JOIN items i ON pi.item_id = i.id
            WHERE UPPER(p.status) NOT IN ('CLOSED', 'CANCELLED')
            ${dateFilter}
            GROUP BY COALESCE(pi.item_name, i.name, 'Unknown Item'), COALESCE(i.sku, '')
            ORDER BY COALESCE(pi.item_name, i.name, 'Unknown Item') ASC
        `, params);

    res.json(result.rows);
  } catch (err) {
    console.error('Error fetching active purchase orders report:', err);
    res.status(500).json({ error: 'Failed to fetch active purchase orders report' });
  }
});

// GET Purchases by Item Report
router.get('/reports/purchases-by-item', async (req, res) => {
  try {
    const db = database.getDb();
    const { from, to } = req.query;
    ...
    const result = await db.query(`
            SELECT COALESCE(pi.item_name, i.name, 'Unknown Item') AS item_name,
                   SUM(pi.quantity) AS quantity_purchased,
                   SUM(pi.total_price) AS amount,
                   CASE WHEN SUM(pi.quantity) > 0
                        THEN ROUND((SUM(pi.total_price) / SUM(pi.quantity))::numeric, 2)
    ...
    console.error('Error fetching purchases by item report:', err);
    res.status(500).json({ error: 'Failed to fetch purchases by item report' });
  }
});
