...
router.post('/', async (req, res) => {
  try {
    const { supplier_id, supplier_name, items, invoice_number, po_number, expected_date, payment_terms, notes, status, date } = req.body;
    ...
    await database.transaction(async (client) => {
      ...
      const purchaseResult = await client.query(`
        INSERT INTO purchases (supplier_id, supplier_name, total_amount, invoice_number, po_number, expected_date, payment_terms, notes, status, date)
...
router.put('/:id', async (req, res) => {
  try {
    const { status, received_date, receiving_notes, supplier_id } = req.body;
    const db = database.getDb();
    ...
    // Ensure supplier linkage if we have a supplier name but no supplier_id yet
    ...
    // Try to backfill supplier_id using supplier_name when missing
    if (!finalSupplierId && finalSupplierName) {
      const supplierLookup = await db.query(
      ...
      if (supplierLookup.rows.length > 0) {
        ...
        finalSupplierId = newSupplier.rows[0].id;
      }
    }
    ...
    if (finalSupplierId !== undefined) {
      ...
      // Also sync supplier_name from DB if available
      if (finalSupplierId) {
        const supplierResult = await db.query('SELECT name FROM suppliers WHERE id = $1', [finalSupplierId]);
        if (supplierResult.rows.length > 0) {
          finalSupplierName = supplierResult.rows[0].name;
    ...
    if (received_date !== undefined) {
      ...
      values.push(received_date);
    }
    if (receiving_notes !== undefined) {
      updates.push(`notes = $${paramIndex++}`);
      values.push(receiving_notes);
    }
    if (updates.length === 0) {
    ...
    if (status === 'received' && existingPurchase.status !== 'received') {
      ...
      for (const purchaseItem of itemsResult.rows) {
        ...
        // CASE 1: New item that needs to be created
        if (!itemId && purchaseItem.is_new === true && purchaseItem.item_name) {
          ...
          if (newItemResult.rows.length > 0) {
            itemId = newItemResult.rows[0].id;
            itemWasCreatedNow = true; // Mark that we just created this item
            // Update purchase_item with the new item_id
        ...
        // Record inventory transaction only if we have an item_id
        if (itemId) {
          await db.query(
